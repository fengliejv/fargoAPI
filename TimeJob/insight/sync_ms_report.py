...
from service.InsightService import get_report_file_basic_not_sync, get_sync_node_time, set_sync_node_time, \
    get_report_file_basic_by_publish_time, get_fargo_insight_file_basic_not_sync, get_sync_record, add_sync_record
...
def sync_ms_report():
    node_time = get_sync_node_time(task_name='sync_ms_report')
    files = get_fargo_insight_file_basic_not_sync(create_time=node_time[0]['node_time'], source='ms')
    for file in files:
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                set_sync_node_time(node_time=file['create_time'], update_time=datetime.now(),
                                   task_name='sync_ms_report')
                print(f"success,article{file['uuid']}")
        except Exception as e:
            print(str(e))
            add_error_log(message=f"Report更新失败,报错:{str(e)}")
