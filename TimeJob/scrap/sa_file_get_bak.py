...
SOURCE = 'sa'
PATH = f"{Config.FILE_STORAGE_PATH}{SOURCE}"
...
def downloadHtml(file_url, local_file_path, header):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            add_error_log(message=f"网页下载失败. 响应状态码:{response.status_code}，下载链接{file_url}")
    except Exception as e:
        add_fatal_log(message=f"下载网页时发生错误:{str(e)}，下载链接{file_url}")
...
def get_sa_report():
    ...
    for item in company_list:
        ...
        while page < 10000:
            ...
            add_info_log(message=f"爬取{item},page:{page},时间戳{time.time()}")
            ...
            try:
                ...
                add_fatal_log(message=f"定时更新{SOURCE}平台公司报告异常,公司名：{item},报告页码：{page},报错:{str(e)}")
