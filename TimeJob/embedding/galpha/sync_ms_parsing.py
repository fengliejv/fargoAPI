...
def sync_ms_parsing():
    node_time = get_consumer_node_time(task_name='sync_ms_parsing', consumer='galpha')
    files = get_file_basic_need_parse(start_time=node_time[0]['node_time'], source='ms')
    for file in files:
        try:
            ...
            with open(file=file['local_save_path'], mode='rb') as fis:
                ...
                if r.status_code != 200:
                    ...
                    add_fatal_log(f"ms Report parsing失败")
                else:
                    ...
                    set_sync_node_time(node_time=file['create_time'], update_time=datetime.now(),
                                       task_name='sync_ms_parsing')
        except Exception as e:
            print(str(e))
            add_error_log(message=f"ms Report Parsing失败,报错:{str(e)}")
            continue
