...
from service.ReportService import get_article_newest_time_by_company, add_error_log, add_fatal_log, add_file_record
...
def downloadHtml(file_url, local_file_path, header):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            add_error_log(message=f"fail:{response.status_code}")
    except Exception as e:
        add_fatal_log(message=f"fail:{str(e)}")


def sync_file_to_insight_article_page(content: str, summary: str, symbol: str, article_id: str, head: str,
                                      viewpoint: str, time: str, name: str, title: str) -> bool:
    try:
        ...
        for question in qs:
            ...
            if r.status_code != 200:
                add_fatal_log(f"update fail")
            print(f"success,article{article_id}")
        return True
    except Exception as e:
        add_fatal_log(f"fail {str(e)}")
        return False
    ...
    for item in company_list:
        ...
        while page < 10000:
            ...
            if len(newest_artcile) <= 0:
                break
            newest_time = newest_artcile[0]['publish_time']
                        ...
                        if res:
                            ...
                            r = requests.post(
                                ...
                                json={
                                    ...
                                    "queries": [{
                                        "sql":
                                            f"SELECT * from \"native-table-d0be72f8-715d-4bd4-975d-20691d09d2ad\" where \"n99aj\"={doc['id']} and \"m9BiN\"='SA'"
                                    }]
                                })
                            ...
                            content = responseHtml.find_all('div', attrs={'data-test-id': 'article-content'})
                            if len(content) <= 0:
                            ...
                            viewpoint = re.findall("{([^{}]*sentiment\",\"attributes\".*?)}", file_content, flags=re.S)
                            viewpoint = str(viewpoint[0])
                            viewpoint = viewpoint[viewpoint.find("{"):len(viewpoint)] + "}"
                            viewpoint = json.loads(viewpoint)
                            
                            stock = item.replace(".", "").replace(" ", "").replace("&", "")
                                ...
                                json={
                                    ...
                                    "mutations": [{
                                        ...
                                        "columnValues": {
                                            ...
                                            "Jww8r": doc['attributes']['title'],
                                            "zpVLQ": content,
                                            "ZicAt": summary,
                                            "LppoE": f'{publish_time}',
                                            "oCPF0": lang,
                                            "m9BiN": "SA",
                                        }
                                    }]
                            ...
                            if r.status_code != 200:
                                add_fatal_log(
                                    message=f"fail：{item},page：{page},error:{r.text}")
                                continue

                            
                            res = sync_file_to_insight_article_page(content=content, summary=summary,
                                                                    symbol=com_dict.get(stock), article_id=id,
                                                                    head=head, viewpoint=viewpoint["type"],
                                                                    time=f"{publish_time}",
                                                                    name=name, title=doc['attributes']['title'])
                            if res:
                                
                                print(f"定时更新{item}公司报告{id},报告页码：{page}")
                page = page + 1
            except Exception as e:
